...
- LMS at https://education.coursery.co
- CMS at https://studio.education.coursery.co
- CDN at https://cdn.education.coursery.co linked to a public read-only S3 bucket named education-coursery-global-storage
- Grafana at https://grafana.education.coursery.co/login
  - user: admin
  - pwd: prom-operator
- public ssh access via a t2.micro Ubuntu 20.04 LTS bastion EC2 instance at bastion.education.coursery.co
- daily data backups archived into a private S3 bucket named prod-coursery-global-mongodb-backup
...
- LMS at https://dev.education.coursery.co
- CMS at https://studio.dev.education-coursery.co
- CDN at https://cdn.dev.education.coursery.co linked to an S3 bucket named dev-coursery-global-storage
- daily data backups archived into an S3 bucket named dev-coursery-global-mongodb-backup
...
.. code-block:: hcl
  ...
  locals {
    platform_name    = "coursery"
    platform_region  = "global"
    root_domain      = "coursery.co.ai"
    aws_region       = "ap-south-1"
    account_id       = "751448075284"
  }
...
.. code-block:: shell
  ...
  ssh bastion.education.coursery.co -i path/to/coursery-global-live-bastion.pem

  mysql -h mysql.education.coursery.co -u root -p

  mongo --port 27017 --host mongo.master.education.coursery.co -u root -p
  mongo --port 27017 --host mongo.reader.education.coursery.co -u root -p

  redis-cli -h redis.primary.education.coursery.co -p 6379
...
Use `this automated Github Actions workflow <https://github.com/crsry/oedo/actions/workflows/tutor_build_image.yml>`_ to build a customized Open edX Docker container based on the latest stable version of Open edX (current nutmeg.2) and
...
Use `this automated Github Actions workflow <https://github.com/crsry/oedo/actions/workflows/tutor_deploy_prod.yml>`_ to deploy your customized Docker container to a Kubernetes Cluster.
...
- **MySQL**. uses `AWS RDS <https://aws.amazon.com/rds/>`_ for all MySQL data, accessible inside the vpc as mysql.education.coursery.co:3306. Instance size settings are located in the `environment configuration file <terraform/environments/prod/env.hcl>`_, and other common configuration settings `are located here <terraform/environments/prod/rds/terragrunt.hcl>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
- **MongoDB**. uses `AWS DocumentDB <https://aws.amazon.com/documentdb/>`_ for all MongoDB data, accessible insid the vpc as mongodb.master.education.coursery.co:27017 and mongodb.reader.education.coursery.co. Instance size settings are located in the `environment configuration file <terraform/environments/prod/env.hcl>`_, and other common configuration settings `are located here <terraform/modules/documentdb>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
- **Redis**. uses `AWS ElastiCache <https://aws.amazon.com/elasticache/>`_ for all Django application caches, accessible inside the vpc as cache.education.coursery.co. Instance size settings are located in the `environment configuration file <terraform/environments/prod/env.hcl>`_. This is necessary in order to make the Open edX application layer completely ephemeral. Most importantly, user's login session tokens are persisted in Redis and so these need to be accessible to all app containers from a single Redis cache. Common configuration settings `are located here <terraform/environments/prod/redis/terragrunt.hcl>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
